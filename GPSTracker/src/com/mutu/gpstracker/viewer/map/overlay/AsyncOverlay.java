package com.mutu.gpstracker.viewer.map.overlay;
...
public abstract class AsyncOverlay extends Overlay implements OverlayProvider
{
   ...
   private void draw(Canvas canvas)
   {
      ...
      if (mActiveBitmap.getWidth() > 1)
      {
         synchronized (mActiveBitmap)
         {
            mLoggerMap.toMapPixels(mActiveTopLeft, mActivePointTopLeft);
...
//      Point pntCenter = new Point();
//      mLoggerMap.toPixels(mLoggerMap.getMapCenter(),pntCenter);
//      canvas.drawColor(Color.BLUE);
//      canvas.drawLine(0, 0, pntCenter.x, pntCenter.y, mPaint);
//      canvas.restore();
   }
...
//   }
   ...
   static class SegmentOsmOverlay extends Overlay
   {
      ...
      @Override
      public boolean onSingleTapUp(MotionEvent e, MapView openStreetMapView)
      {
         int x = (int) e.getX();
         int y = (int) e.getY();
