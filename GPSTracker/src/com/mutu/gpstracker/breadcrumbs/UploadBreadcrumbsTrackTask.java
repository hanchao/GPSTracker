 ...
 *
 */
package com.mutu.gpstracker.breadcrumbs;
...
import com.mutu.gpstracker.actions.tasks.GpxCreator;
import com.mutu.gpstracker.actions.tasks.XmlCreator;
import com.mutu.gpstracker.actions.utils.ProgressListener;
import com.mutu.gpstracker.adapter.BreadcrumbsAdapter;
import com.mutu.gpstracker.db.GPStracking.MetaData;
import com.mutu.gpstracker.util.MultipartStreamer;
...
public class UploadBreadcrumbsTrackTask extends GpxCreator
{
    ...
    * @param name
    */
   public UploadBreadcrumbsTrackTask(Context context, BreadcrumbsService adapter, ProgressListener listener,
         OAuthConsumer consumer, Uri trackUri, String name)
   {
   ...
   protected Uri doInBackground(Void... params)
   {
      ...
      if (isCancelled())
      {
         ...
         handleError(mContext.getString(R.string.taskerror_breadcrumbs_upload), new IOException(
               "Fail to execute request due to canceling"), text);
      }
      ...
      try
      {
         cursor = mContext.getContentResolver().query(metadataUri, new String[] { MetaData.KEY, MetaData.VALUE }, null,
               null, null);
      ...
      if (statusCode == 200 || statusCode == 201)
      {
         ...
         {
            handleError(mContext.getString(R.string.taskerror_breadcrumbs_upload), new IOException(
                  "Unable to retrieve URI from response"), responseText);
         }
      }
      else
      {
         ...
         handleError(mContext.getString(R.string.taskerror_breadcrumbs_upload), new IOException("Status code: "
   ...
   protected void onPostExecute(Uri result)
   {
      ...
      mService.getBreadcrumbsTracks().addTrack(bcTrackId, mName, Integer.valueOf(mBundleId), mDescription, null, null,
            null, mIsPublic, null, null, null, null, null);
